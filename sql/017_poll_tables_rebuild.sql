-- Rebuild poll_* tables if an older incompatible shape exists (016 used IF NOT EXISTS).

drop table if exists public.poll_crosstabs cascade;
drop table if exists public.poll_questions cascade;
drop table if exists public.poll_surveys cascade;

create table public.poll_surveys (
...
create index poll_questions_survey_id_idx on public.poll_questions (survey_id);

create table public.poll_crosstabs (
  id bigint generated always as identity primary key,
...
create index poll_crosstabs_question_id_idx on public.poll_crosstabs (question_id);
create index poll_crosstabs_segment_type_idx on public.poll_crosstabs (segment_type);
